...
----
    local function GetStringsTable(prefab_name)
        local temp_name = "sword_fairy_com_map_jumper"
...
local SWORD_FAIRY_BLINK_MAP = Action({ priority=999, customarrivecheck=ArriveAnywhere, rmb=true, mount_valid=false, map_action=true, })  
SWORD_FAIRY_BLINK_MAP.id = "SWORD_FAIRY_BLINK_MAP"
SWORD_FAIRY_BLINK_MAP.strfn = function(act) --- 客户端检查是否通过,同时返回显示字段
    if act.doer then
        return "DEFAULT"
    end
end

SWORD_FAIRY_BLINK_MAP.fn = function(act)    --- 只在服务端执行~
    ...
    if doer then
        if doer.components.sword_fairy_com_map_jumper then
            doer.components.sword_fairy_com_map_jumper:CastPreSpell(Vector3(x,y,z))
        end
        
        if doer.sg then            
                doer.sg:GoToState("portal_jumpin_sword_fairy", {dest = act_pos, from_map = true,})
                doer:DoTaskInTime(1,function()
                    if doer:GetDistanceSqToPoint(act_pos:Get()) < 3 then
                        doer.components.sword_fairy_com_map_jumper:CastSpell(Vector3(x,y,z))
                    end
                end)
        end
...
AddAction(SWORD_FAIRY_BLINK_MAP)
STRINGS.ACTIONS.SWORD_FAIRY_BLINK_MAP = {
    DEFAULT = GetStringsTable()["DEFAULT"] or "跳跳跳跳跳",
}
STRINGS.SWORD_FAIRY_BLINK_MAP = ACTIONS.SWORD_FAIRY_BLINK_MAP.mod_name

local BLINK_MAP_MUST = { "CLASSIFIED", "globalmapicon", "fogrevealer" }
ACTIONS_MAP_REMAP[ACTIONS.SWORD_FAIRY_BLINK_MAP.code] = function(act, targetpos)
    -----------------------------------------------------
    ...
    if not doer.replica.sword_fairy_com_map_jumper:Test(Vector3(targetpos.x,targetpos.y,targetpos.z)) then
        return nil
    end

    -- print(" +++++++++++++++++++ ",targetpos,act.invobject)
    local act_remap = BufferedAction(doer, nil, ACTIONS.SWORD_FAIRY_BLINK_MAP, act.invobject, targetpos)
    return act_remap
end
...
----------- SG
    ...
    AddStategraphState("wilson",State{
        name = "portal_jumpin_sword_fairy",
        ...
        onenter = function(inst, data)
            ...
            if inst.components.playercontroller ~= nil then
                inst.components.playercontroller:RemotePausePrediction(10)
            end
        ...
        ontimeout = function(inst)
            ...
            inst.sg:GoToState("portal_jumpout_fwd_in_pdt", {dest = inst.sg.statemem.dest, from_map = inst.sg.statemem.from_map})
        end,
    ...
    AddStategraphState("wilson",State{
        name = "portal_jumpout_fwd_in_pdt",
